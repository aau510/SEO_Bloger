#!/usr/bin/expect -f
# 文件名：deploy-advanced-proxy-v2.sh
# 用途：部署高级代理服务器（修复权限问题）
...
puts "📦 步骤1: 上传代理文件到用户目录..."
spawn scp "scripts/advanced-proxy-server.js" "scripts/package.json" "$user@$server:~/"
expect {
    "password:" {
        send "$password\r"
        exp_continue
...
expect "$ " {
    ...
    expect "$ "
    send "sudo mv ~/advanced-proxy-server.js ~/package.json $remote_dir/\r"
    expect "$ "
